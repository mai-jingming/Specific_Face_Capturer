...
class SFC:
    ...
    def process(self):
        ...
        self.tg_encoder()
        vr = VideoReader(self.vid_path, num_threads=0)
        frame_num = len(vr)
        print("总帧数为：", frame_num)
        height, width = vr[0].shape[0], vr[0].shape[1]
        print(f'视频分辨率为：{width}×{height}')

        for frame_id in tqdm(range(0, frame_num, self.interval)):
            frame = vr[frame_id]
            if not frame:
                continue
            frame = frame.asnumpy()
...
if __name__ == '__main__':
    ...
    vid_decoder = input('请输入要用的视频解码器，输入1为decord，输入2为opencv。decord比opencv在连续帧读取上快了128%~138%，但opencv可以跳过坏帧而不停止程序\n')
